...
namespace Microsoft.Health.Fhir.Api.UnitTests.Controllers
{
    public class BulkImportControllerTests
    {
        ...
        [Theory]
        [MemberData(nameof(ValidBody), MemberType = typeof(BulkImportControllerTests))]
        public async Task GivenABulkImportRequest_WithValidBody_ThenBulkImportCalledWithCorrectConfiguration(BulkImportRequestConfiguration body)
        {
            var bulkImportController = GetController(new BulkImportJobConfiguration() { Enabled = true });
            _mediator.BulkImportAsync(Arg.Any<Uri>(), Arg.Any<BulkImportRequestConfiguration>(), Arg.Any<CancellationToken>()).Returns(Task.FromResult(CreateBulkImportResponse()));

            // _mediator.Send(Arg.Any<CreateBulkImportRequest>()).Returns(Task.FromResult(CreateBulkImportResponse()));
            await bulkImportController.BulkImport(body);
            await _mediator.Received().BulkImportAsync(
                Arg.Any<Uri>(),
                Arg.Is<BulkImportRequestConfiguration>(r => r.ToString().Equals(body.ToString())),
                Arg.Any<CancellationToken>());
            await _mediator.Received().Send(
                Arg.Is<CreateBulkImportRequest>(r => r.RequestConfiguration.ToString().Equals(body.ToString())));
